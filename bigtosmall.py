...
def split_image(image_path, output_dir, tile_size):
    ...
    for i in range(0, img_width, tile_size):
        for j in range(0, img_height, tile_size):
            ...
            img_cropped.save(os.path.join(output_dir, f"{base_name}_{i}_{j}.png"))


def batch_split_images(input_dir, output_dir, tile_size):
    os.makedirs(output_dir, exist_ok=True)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Split large images into fixed-size tiles.")
...
def main():
    args = parse_args()
    batch_split_images(args.input_dir, args.output_dir, args.tile_size)
